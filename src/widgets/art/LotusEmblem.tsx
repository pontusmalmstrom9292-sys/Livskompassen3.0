...
export function LotusEmblem({ className = '', style, size = 64 }: LotusEmblemProps) {
  ...
  return (
    <svg
      ...
      width={size}
      height={size}
      className={['cw-lotus', className].filter(Boolean).join(' ')}
      style={style}
      aria-hidden
      focusable="false"
    >
      ...
      <circle cx="50" cy="50" r="42" fill="#070d18" stroke={`url(#${lotusGoldRim})`} strokeWidth="1.5" />
      <circle cx="50" cy="50" r="36" fill="none" stroke="#1c2d4a" strokeWidth="1" />
      <path
        d="M50 20 C60 38, 62 55, 50 78 C38 55, 40 38, 50 20 Z"
        fill={`url(#${lotusBlue})`}
        stroke="#93c5fd"
        strokeWidth="1"
      />
      <path
        d="M50 28 C70 40, 75 60, 50 78 C25 60, 30 40, 50 28 Z"
        fill={`url(#${lotusBlue})`}
        opacity="0.85"
      />
      <path
        d="M50 35 C80 48, 80 70, 50 78 C20 70, 20 48, 50 35 Z"
        fill={`url(#${lotusBlue})`}
        opacity="0.6"
      />
