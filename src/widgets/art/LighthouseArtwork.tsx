...
export type LighthouseArtworkProps = { className?: string; style?: CSSProperties };
...
export function LighthouseArtwork({ className = '', style }: LighthouseArtworkProps) {
  ...
  return (
    <svg viewBox="0 0 320 170" className={['cw-art-lighthouse', className].filter(Boolean).join(' ')} style={{ display: 'block', width: '100%', height: 'auto', borderRadius: 16, ...style }} aria-hidden focusable="false">
      <defs>
        <linearGradient id={lhSky} x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%" stopColor="#081021" /><stop offset="60%" stopColor="#101d36" /><stop offset="100%" stopColor="#182744" />
        </linearGradient>
        ...
        <linearGradient id={cliffGrad} x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor="#1a273b" /><stop offset="100%" stopColor="#080e19" />
        </linearGradient>
      </defs>
      ...
      <g className="cw-art-lighthouse__beam"><polygon points="230,58 -20,10 -20,120" fill={`url(#${lightBeam})`} /></g>
      <circle cx="230" cy="58" r="45" fill={`url(#${beaconGlow})`} />
      <path d="M0 125 Q60 120 120 130 T240 125 T320 130 L320 170 L0 170 Z" fill="#080e1a" opacity="0.95" />
