...
export function FacetedCompassRose({ angle = 28, className = '', style, size }: FacetedCompassRoseProps) {
  ...
  const merged: CSSProperties = { width: size, height: size, display: 'block', ...style };
  return (
    <svg viewBox="0 0 200 200" className={className} style={merged} aria-hidden focusable="false">
      ...
      <circle cx="100" cy="100" r="94" fill={WidgetPalette.deepSpaceBlue} stroke={`url(#${goldLight})`} strokeWidth="3.5" filter={`url(#${goldGlow})`} />
      ...
      {[0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330].map((deg) => (
        <line key={deg} x1="100" y1="26" x2="100" y2={deg % 90 === 0 ? '35' : '30'} stroke={deg % 90 === 0 ? WidgetPalette.premiumGoldLight : '#3d5270'} strokeWidth={deg % 90 === 0 ? 2.5 : 1} transform={`rotate(${deg} 100 100)`} />
      ))}
      <text x="100" y="21" fill={WidgetPalette.premiumGoldLight} fontSize="12" fontWeight="800" textAnchor="middle" letterSpacing="1">N</text>
      <text x="100" y="189" fill="#597196" fontSize="10" fontWeight="bold" textAnchor="middle">S</text>
      <text x="15" y="104" fill="#597196" fontSize="10" fontWeight="bold" textAnchor="middle">W</text>
      <text x="185" y="104" fill="#597196" fontSize="10" fontWeight="bold" textAnchor="middle">E</text>
      ...
      <circle cx="100" cy="100" r="10" fill={`url(#${goldDark})`} stroke={WidgetPalette.premiumGoldLight} strokeWidth="1.5" />
