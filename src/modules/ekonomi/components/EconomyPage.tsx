...
import { EconomyPeriodSummary } from './EconomyPeriodSummary';
...
type EconomyTab = 'tid' | 'pengar' | 'logg';

const TABS: TabBarItem<EconomyTab>[] = [
  { id: 'tid', label: 'Tid' },
...
export function EconomyPage({ embedded = false }: EconomyPageProps) {
  ...
  const [tab, setTab] = useState<EconomyTab>('pengar');
  const [transactions, setTransactions] = useState<
    { id: string; label: string; amountSek: number; category: string; createdAt: string }[]
  >([]);
  const [periodSummary, setPeriodSummary] = useState<PeriodEconomySummary | null>(null);
  ...
  return (
    <div className="space-y-4">
      <TabBar tabs={TABS} active={tab} onChange={(id) => setTab(id)} variant="cluster" />

      {error && <p className="text-sm text-danger">{error}</p>}

      {tab === 'tid' && <EconomyTidPanel />}

      {tab === 'logg' && <EconomyLogPanel onChanged={() => void reload()} />}
      ...
      {tab === 'pengar' && (
        <>
          ...
          <EconomyPeriodSummary summary={periodSummary} loading={loading} />
          <EconomyPayslipCard />
          ...
          <BentoCard
            ...
            description="Enkla mål — inga grafer"
          >
            {savings.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-2">
              <input
                className="input-glass min-w-[120px] flex-1"
          ...
          <BentoCard
            ...
            description="Veckopeng och matlåda (WORM)"
          >
            {saving && (
