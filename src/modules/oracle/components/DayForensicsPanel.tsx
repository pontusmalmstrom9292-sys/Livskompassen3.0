...
export const DayForensicsPanel: React.FC<DayForensicsPanelProps> = ({ dataPoint, onClose }) => {
  ...
  return (
    <div className="bg-surface-2 border border-white/10 rounded-2xl p-6 backdrop-blur-xl relative z-50 mt-8 shadow-2xl motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-4 duration-300">
      <button 
        onClick={onClose}
      ...
      <div className="mb-6">
        ...
        <div className="flex gap-4 mt-3 text-sm">
          <span className="px-2 py-1 rounded bg-red-500/10 text-danger border border-red-500/20">
            Stress: {dataPoint.stressLevel}
          </span>
          <span className="px-2 py-1 rounded bg-success/10 text-success border border-success/20">
      ...
      {isLoading ? (
        <HubPanelSkeleton label="Laddar dagsanalys…" lines={3} />
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-black/30 rounded-xl p-5 border border-white/5">
            ...
            {vaultEntries.length === 0 ? (
              <EmptyState className="bg-transparent shadow-none" message="Inga bevis registrerade denna dag." />
            ) : (
          ...
          <div className="bg-black/30 rounded-xl p-5 border border-white/5">
            ...
            {timeEntries.length === 0 ? (
               <EmptyState className="bg-transparent shadow-none" message="Ingen tid loggad denna dag." />
            ) : (
               <div className="space-y-3">
                 {timeEntries.map((entry, idx) => (
                   <div key={idx} className="flex justify-between items-center bg-white/5 rounded-lg p-3 text-sm">
                     <span className="text-text-muted">{entry.client || 'Okänd klient'} - {entry.project || 'Ospec.'}</span>
                 ...
                 <div className="pt-3 border-t border-white/10 flex justify-between items-center font-medium mt-2">
                   <span className="text-text-muted">Totalt</span>
            ...
            <div className="mt-6 pt-5 border-t border-white/5">
              <h4 className="text-sm font-semibold text-accent-secondary mb-2">⚡ Automatisk slutsats</h4>
