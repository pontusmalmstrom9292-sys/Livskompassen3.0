      ...
      <div className="mb-2 flex items-center justify-between gap-2 px-2 pt-2">
        <div className="flex flex-1 gap-1 overflow-x-auto rounded-t-xl border-b border-border/30 bg-surface-2 p-1 calm-scroll-island">
          {visibleTabs.map(({ id, label, icon: Icon }) => (
            <button
              ...
              className={clsx(
                'flex min-h-[var(--ds-touch-target)] shrink-0 items-center justify-center gap-1.5 rounded-lg px-2.5 py-2.5 text-[11px] font-medium transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/40',
                activeTab === id
        ...
        {isEconomyAdvancedUnlocked && (
          <ButtonLink
            ...
            variant="ghost"
            size="icon"
            className="flex min-h-11 items-center justify-center border border-accent/20 bg-accent/5 hover:bg-accent/10 hover:text-accent transition-colors rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/40"
