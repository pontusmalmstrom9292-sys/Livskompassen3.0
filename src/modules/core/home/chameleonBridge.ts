...
export type ChameleonModule = 'dagbok' | 'planering' | 'mabra' | 'familjen';

export type ChameleonTarget = {
  zone: ChameleonZoneId;
  module: ChameleonModule;
  mode: string;
};

export function getDefaultTarget(zone: ChameleonZoneId): ChameleonTarget {
  switch (zone) {
    case 'hjartat':
...
export function resolveCardToChameleon(card: HemV3DevCard): ChameleonTarget {
...
export function isModeValidForZone(zone: ChameleonZoneId, module: ChameleonModule): boolean {
