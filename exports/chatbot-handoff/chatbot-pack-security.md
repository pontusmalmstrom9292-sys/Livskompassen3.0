...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    // WORM hash chain — append-only via Admin SDK
    match /worm_hash_chain/{docId} {
      allow read: if isOwner();
      allow create, update, delete: if false;
    }

    // DCAP eskalering — rekommenderade åtgärder (append-only)
    match /dcap_escalation_actions/{docId} {
      allow read: if isOwner();
      allow create, update, delete: if false;
    }
