/**
 * v5 — 5 stilar (G1–G5) × 14 hub-kategorier. Output → 2026-05-29-gold-hub-v5/
 */
...
const outDir = join(root, 'docs/design/icons-proposals/2026-05-29-gold-hub-v5');

const STYLES = [
  { id: 'g1-wire', name: 'Gold Wire', mode: 'flat', family: 'wire' },
  { id: 'g2-emboss-3d', name: 'Gold Emboss 3D', mode: '3d', family: 'helros' },
  { id: 'g3-aurora', name: 'Aurora Spectral', mode: '3d', family: 'aurora' },
  { id: 'g4-ember', name: 'Sacred Ember', mode: '3d', family: 'eldnal' },
  { id: 'g5-obsidian', name: 'Obsidian Glass', mode: '3d', family: 'obsidian' },
];

function slugStyle(style) {
  return style.id;
}
...
const CATEGORIES = [
  ...
  { dir: 'hamnBiff', prefix: 'Hb', key: 'hamnBiff' },
  { dir: 'valv', prefix: 'V', key: 'valv' },
  { dir: 'dagbok', prefix: 'J', key: 'dagbok' },
  { dir: 'planering', prefix: 'P', key: 'planering' },
  { dir: 'mabra', prefix: 'A', key: 'mabra' },
  { dir: 'kompis', prefix: 'K', key: 'kompis' },
  { dir: 'projekt', prefix: 'Pr', key: 'projekt' },
  { dir: 'arbetsliv', prefix: 'Ar', key: 'arbetsliv' },
  { dir: 'rutiner', prefix: 'R', key: 'rutiner' },
  { dir: 'ekonomi', prefix: 'E', key: 'ekonomi' },
  { dir: 'utveckling', prefix: 'U', key: 'utveckling' },
  { dir: 'kunskap', prefix: 'Kn', key: 'kunskap' },
];
...
const GLYPHS = {
  ...
  hamnBiff: (g, f, gl) => `
  <path d="M24 10 L34 14 V24 Q34 34 24 38 Q14 34 14 24 V14 Z" stroke="url(#${g})" stroke-width="0.75" fill="url(#${g})" fill-opacity="0.12"/>
  <path d="M11 34 H37" stroke="url(#${g})" stroke-width="0.7" stroke-linecap="round"/>
  <path d="M18 22 V34 M30 22 V34" stroke="url(#${g})" stroke-width="0.65"/>
  <path d="M24 14 L27 20 L24 18 L21 20 Z" fill="url(#${f})" filter="url(#${gl})"/>`,
  kompis: (g, f, gl) => `
  <polygon points="24,9 26.5,17.5 35,17.5 28,22.5 30.5,31 24,26.5 17.5,31 20,22.5 13,17.5 21.5,17.5" fill="none" stroke="url(#${g})" stroke-width="0.7"/>
  <polygon points="24,13 25.5,18 30,18 27,21 28,26 24,23.5 20,26 21,21 18,18 22.5,18" fill="url(#${f})" fill-opacity="0.75" filter="url(#${gl})"/>`,
  projekt: (g, f, gl) => `
  <path d="M12 14 H30 V32 H12 Z" stroke="url(#${g})" stroke-width="0.65" fill="url(#${g})" fill-opacity="0.1"/>
  <path d="M12 14 H22 V20 H30" stroke="url(#${g})" stroke-width="0.55" fill="none"/>
  <path d="M16 24 H28 M16 27 H25" stroke="url(#${g})" stroke-width="0.4" stroke-linecap="round" opacity="0.5"/>
  <circle cx="27" cy="17" r="1.2" fill="url(#${f})" filter="url(#${gl})"/>`,
  arbetsliv: (g, f, gl) => `
  <circle cx="24" cy="24" r="10" stroke="url(#${g})" stroke-width="0.65" fill="none"/>
  <path d="M24 14 V24 L30 28" stroke="url(#${g})" stroke-width="0.75" stroke-linecap="round"/>
  <circle cx="24" cy="24" r="1.5" fill="url(#${f})" filter="url(#${gl})"/>`,
};

function wireChromePlate(ids) {
  const { g } = ids;
  return `
  <circle cx="24" cy="24" r="22" fill="none" stroke="url(#${g})" stroke-width="0.8" stroke-opacity="0.88"/>
  <circle cx="24" cy="24" r="18" stroke="url(#${g})" stroke-opacity="0.4" stroke-width="0.55" fill="none"/>
  <circle cx="24" cy="24" r="14" stroke="url(#${g})" stroke-opacity="0.22" stroke-width="0.45" fill="none"/>
  ${tickRing(g)}`;
}

/** D1-skiva för chrome — ringar + ticks, ingen 8-spets ros (den är bara D1-kompass). */
function d1ChromePlate(ids, style) {
  if (style?.family === 'wire') return wireChromePlate(ids);
  const { d, g } = ids;
  return `
...
function styleLayer(style, ids) {
  ...
  if (fam === 'wire') {
    return `
  <circle cx="24" cy="24" r="20" stroke="url(#${g})" stroke-opacity="0.15" stroke-width="0.35" fill="none"/>`;
  }
...
/** Chrome/meny/hero — platta + kategori-symbol + stil-overlay. */
function buildChromeIcon(catKey, style, catPrefix) {
  const id = uid(['v5', catKey, style.id]);
  const ids = defsBlock(id, style.mode);
  const glyph = GLYPHS[catKey](ids.g, ids.f, ids.gl);
  const overlay = styleLayer(style, ids);
  const plate = d1ChromePlate(ids, style);
  const hub =
    style.family === 'wire'
      ? ''
      : `<circle cx="24" cy="24" r="2.5" fill="url(#${ids.g})" stroke="#0a0a0a" stroke-width="0.5"/>`;
  const label = `${catPrefix} — ${style.name} — ${catKey}`;
...
function main() {
  ...
  for (const style of STYLES) {
    const styleDir = join(outDir, style.id);
    mkdirSync(styleDir, { recursive: true });
    for (const cat of CATEGORIES) {
      const fn = `${cat.key}.svg`;
      writeFileSync(
        join(styleDir, fn),
        buildChromeIcon(cat.key, style, cat.prefix),
        'utf8',
      );
      count++;
    }
  ...
  const previewRows = STYLES.map(
    (s) =>
      `<tr><th>${s.name}</th>${CATEGORIES.map((c) => `<td><img src="${s.id}/${c.key}.svg" width="48" height="48" alt="${c.key}"/></td>`).join('')}</tr>`,
  ).join('\n');
  const previewHead = CATEGORIES.map((c) => `<th>${c.key}</th>`).join('');
  writeFileSync(
    join(outDir, 'preview.html'),
    `<!DOCTYPE html><html lang="sv"><head><meta charset="utf-8"/><title>Hub v5 G1–G5</title>
<style>body{background:#020617;color:#e2e8f0;font-family:system-ui;padding:1rem}
table{border-collapse:collapse}td,th{border:1px solid rgba(212,175,55,.2);padding:.35rem;text-align:center}
th{font-size:.65rem;font-weight:500}</style></head><body>
<h1>Gold hub icons v5</h1><table><thead><tr><th>Stil</th>${previewHead}</tr></thead><tbody>${previewRows}</tbody></table></body></html>`,
    'utf8',
  );

  console.log(`[generate_gold_hub_icons_v5] Wrote ${count} SVGs → ${outDir}`);
}
